"""
COT Dashboard — Commitment of Traders Visualizer  v4.1
Fuente: CFTC (Commodity Futures Trading Commission)

Correcciones:
- Compatibilidad con Streamlit actual:
    * Reemplazo de width='stretch' por use_container_width=True
- Lectura de ZIP/CSV más robusta
- Mejor manejo de errores
- Compatibilidad con distintos formatos de fecha/nombre de columnas
"""
...
def _safe_numeric(series_or_value, index):
...
def fetch_year(year: int, prefix: str):
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(r.content)) as z:
            ...
            if not names:
                return pd.DataFrame(), f"❌ {year}: ZIP vacío — {url}"

            fname = sorted(names)[-1]
            ...
            for encoding in ("utf-8", "latin-1", "cp1252"):
                try:
                    df = pd.read_csv(io.BytesIO(raw_bytes), low_memory=False, encoding=encoding)
                    break
                except Exception:
                    continue

            if df is None:
                return pd.DataFrame(), f"❌ {year}: No se pudo leer el CSV dentro del ZIP — {url}"

        return df, f"✅ {year}: {len(df):,} filas — {url}"

    except requests.exceptions.ConnectionError as e:
        return pd.DataFrame(), f"❌ {year}: Conexión fallida — {str(e)[:120]}"
    except requests.exceptions.Timeout:
        return pd.DataFrame(), f"❌ {year}: Timeout — {url}"
...
def load_cot(years: list, prefix: str):
    frames, logs = [], []
    for y in years:
...
def available_markets(df: pd.DataFrame) -> list:
    nc = _name_col(df)
    if nc is None or df.empty:
        return []
    return sorted(df[nc].dropna().astype(str).unique().tolist())
...
def parse_cot(df: pd.DataFrame, market_search: str, rcfg: dict) -> pd.DataFrame:
    ...
    name_series = df[nc].astype(str).str.upper()
    mask = name_series == str(market_search).upper()

    if mask.sum() == 0:
        tokens = [t for t in str(market_search).upper().split() if len(t) > 1]
        mask = name_series.apply(lambda x: all(t in x for t in tokens))
    ...
    df_m["OI"] = _safe_numeric(df_m.get("Open_Interest_All"), df_m.index)
    df_m["NC_Long"] = _safe_numeric(df_m.get(rcfg["long_nc"]), df_m.index)
    df_m["NC_Short"] = _safe_numeric(df_m.get(rcfg["short_nc"]), df_m.index)
    df_m["CM_Long"] = _safe_numeric(df_m.get(rcfg["long_cm"]), df_m.index)
    df_m["CM_Short"] = _safe_numeric(df_m.get(rcfg["short_cm"]), df_m.index)
    df_m["NR_Long"] = _safe_numeric(df_m.get(rcfg["long_nr"]), df_m.index)
    df_m["NR_Short"] = _safe_numeric(df_m.get(rcfg["short_nr"]), df_m.index)
    ...
    cols = [
        "Date", "OI",
        "NC_Long", "NC_Short", "Net_NC",
        "CM_Long", "CM_Short", "Net_CM",
        "NR_Long", "NR_Short", "Net_NR",
    ]
    return df_m[cols].reset_index(drop=True)
...
def cot_index(series: pd.Series, window: int = 52) -> pd.Series:
    ...
    rng = mx - mn
    return pd.Series(
        np.where(rng != 0, (series - mn) / rng * 100, 50.0),
        index=series.index,
    )
...
def chart_net_cot(df, window, nc_label, cm_label):
    ...
    fig.add_trace(
        go.Bar(
            ...
            name=nc_label,
            opacity=0.8,
            marker_color=["#3fb950" if v >= 0 else "#f85149" for v in nc],
        ),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            ...
            y=nc.rolling(4).mean(),
            name="MA 4s",
            line=dict(color="#f0e68c", width=1.8),
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            name=cm_label,
            opacity=0.8,
            marker_color=["#79c0ff" if v >= 0 else "#ffa657" for v in cm],
        ),
        row=2,
        col=1,
    )
...
def signals(df, window):
    if len(df) < 6:
        ...
        return {
            ...
            "nc_trend": "n/d",
            "cm_trend": "n/d",
            "cot_index": cot_index(df["Net_NC"], window).iloc[-1] if len(df) else 50.0,
            "oi": lat["OI"],
            "oi_change": 0.0,
    ...
    oi_chg = _safe_pct_change(lat["OI"], prv["OI"])

    sig, cls = (
        ("ALCISTA", "signal-bull") if ci >= 75 else
...
with st.sidebar:
    st.markdown(
        "<div style='font-family:IBM Plex Mono;font-size:1.05rem;font-weight:600;color:#58a6ff;padding:8px 0 16px'>⬡ COT Dashboard</div>",
        unsafe_allow_html=True,
    )

    market_keys = list(MARKETS.keys())
    default_market = "EUR/USD  ·  Euro FX" if "EUR/USD  ·  Euro FX" in market_keys else market_keys[0]
    ...
    st.markdown(
        f"<div class='info-box'>{rcfg['description']}<br><br>"
        f"<strong>Speculators:</strong> {rcfg['nc_label']}<br>"
        f"<strong>Hedgers:</strong> {rcfg['cm_label']}</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div style='font-family:IBM Plex Mono;font-size:.65rem;color:#8b949e;margin-top:12px'>Datos: CFTC · Actualización viernes</div>",
        unsafe_allow_html=True,
    )
...
st.markdown(
    f"<div class='subtitle'>CFTC · {market_key.strip()} · {report_key} · COT window {cot_win}w</div>",
    unsafe_allow_html=True,
)
...
with st.expander("📡 Log de descarga CFTC", expanded=not has_data):
    ...
    name_col = _name_col(raw)
    if not raw.empty and name_col:
        st.markdown(f"`Total filas: {len(raw):,} · Mercados disponibles: {raw[name_col].nunique()}`")
...
if df.empty:
    ...
    if not raw.empty:
        ...
        auto_hits = [
            m for m in avail
            if m.upper() == str(market_search).upper() or any(t in m.upper() for t in tokens)
        ]

        if auto_hits:
            st.markdown(f"#### 🔍 Posibles coincidencias para `{market_search}`")
            st.dataframe(
                pd.DataFrame({"Nombre exacto en CFTC": auto_hits}),
                use_container_width=True,
            )

        st.markdown(f"#### 📋 Todos los mercados ({len(avail)}) — descarga el CSV")
        csv_all = pd.DataFrame({"market_name": avail}).to_csv(index=False).encode("utf-8")
        ...
        q = st.text_input(
            ...
            placeholder="EURO, PESO, GOLD, S&P, CRUDE...",
        )
        hits = [m for m in avail if q.upper() in m.upper()] if q else avail

        st.dataframe(
            pd.DataFrame({"Nombre en CFTC": hits[:400]}),
            use_container_width=True,
        )
    else:
        st.warning("Descarga fallida. Revisa el log superior para ver el error exacto.")
...
with c1:
    st.markdown(
        f"""<div class='metric-card'>
            ...
            <div class='metric-delta neutral'>{s["last_date"]}</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
with c2:
    ...
    st.markdown(
        f"""<div class='metric-card'>
            ...
            <div class='metric-delta neutral'>{s["nc_trend"]}</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
with c3:
    ...
    st.markdown(
        f"""<div class='metric-card'>
            ...
            <div class='metric-value {cls}'>{v:+,.0f}</div>
            <div class='metric-delta neutral'>{s["cm_trend"]}</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
with c4:
    ...
    st.markdown(
        f"""<div class='metric-card'>
            ...
            <div class='metric-delta neutral'>Ventana {cot_win}w</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
with c5:
    ...
    st.markdown(
        f"""<div class='metric-card'>
            ...
            <div class='metric-delta {cls}'>{oic:+.1f}% (5s)</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
st.plotly_chart(
    chart_net_cot(df, cot_win, rcfg["nc_label"], rcfg["cm_label"]),
    use_container_width=True,
...
with col_l:
    ...
    st.plotly_chart(
        chart_gross(df, rcfg["nc_label"], rcfg["cm_label"]),
        use_container_width=True,
        config={"displayModeBar": False},
    )
...
with st.expander("📋 Datos crudos"):
    ...
    styled_df = df.sort_values("Date", ascending=False).style.format({c: "{:,.0f}" for c in num_cols})
...
st.markdown(
    """
    <div style='text-align:center;font-family:IBM Plex Mono;font-size:.68rem;color:#8b949e'>
    Datos públicos CFTC · Actualización semanal (viernes) · Solo informativo
    </div>
    """,
    unsafe_allow_html=True,
)
